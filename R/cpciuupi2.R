...
#'
#'@param gam A value of gamma or vector of gamma values at which the coverage
...
#'  \code{bsciuupi2}
#'
#'@return The value(s) of the coverage probability of the Kabaila & Giri (2009)
#'  CIUUPI at \code{gam}.
#'
...
cpciuupi2 <- function(gam, bsvec, alpha, m, rho, natural = 1){
  ...
  b.spl <- spline_b(bsvec, n.ints, knots.all, t.alpha, natural)
  ...
  for(i in 1:length(gam)){
    res[i] <- compute_cov_trapez(gam[i], rho, knots, alpha,
                        t.alpha, nodes, weights, b.spl, s.spl,
                        wvec, psinu.zvec, h, cons)
  }
