#'Compute the second definition of the scaled expected length of the Kabaila &
#'Giri (2009) CIUUPI
#'
...
#'
#'@param gam A value of gamma or vector of gamma values at which the second
...
#'  \code{bsciuupi2}
#'
#'@return The value(s) of the second definition of the scaled expected length of
...
sel2ciuupi2 <- function(gam, bsvec, alpha, m, rho, natural = 1){
  ...
  for(i in 1:length(gam)){
    res[i] <- compute_sel2_trapez(gam[i], knots, t.alpha, nodes, weights,
